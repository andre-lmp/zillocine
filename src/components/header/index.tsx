...
export default function Header() {
  ...
  useEffect(() => {
    navLinks.current.forEach((link) => {
      if (
        ["/series", "/movies", "/search", "/", "/favorites"].includes(
          currentPathName,
        )
      ) {
        if (currentPathName === `/${link?.id}`) {
          if (link) {
            ...
            if (
              navBarCoordinates?.left &&
              linkCoordinates.left &&
              indicatorBar.current
            ) {
              const activeLinkCoordinates =
                linkCoordinates.left - navBarCoordinates.left;
      ...
      indicatorBar.current &&
        Object.assign(indicatorBar.current?.style, { width: "0px" });
      link && Object.assign(link?.style, { color: "#d4d4d4" });
    });
  }, [currentPathName]);
  ...
  return (
    <MobileMenu>
      <header
        ...
        className="fixed top-0 left-1/2 -translate-x-1/2 h-36 md:h-auto z-20 w-full before:w-full before:absolute before:top-0 before:left-0 before:h-full before:bg-gradient-to-b from-black to-transparent before:-z-10 max-w-[2200px] ease-linear duration-200"
      >
        <nav
          ref={scrollDiv}
          className="w-full flex flex-row justify-between items-center px-4 py-5 md:px-6 md:py-0 h-20 lg:px-8"
        >
          ...
          <div>
            <h1 className="text-2xl font-raleway font-bold md:text-[25px] ">
              IFDB
            </h1>
          </div>
          ...
          <nav className="flex flex-row items-center font-noto_sans font-semibold">
            <ul
              ref={navBar}
              className="flex flex-row w-fit items-center *:text-neutral-300 justify-end gap-x-10 relative"
            >
              <li
                key="li-element-1"
                id=""
                className="hidden md:flex text-lg cursor-pointer 2xl:text-[19px]"
                ref={(e) => {
                  navLinks.current[0] = e;
                }}
              >
              ...
              <li
                key="li-element-2"
                id="movies"
                className="hidden md:inline text-lg cursor-pointer 2xl:text-[19px]"
                ref={(e) => {
                  navLinks.current[1] = e;
                }}
              >
              ...
              <li
                key="li-element-4"
                id="favorites"
                className="hidden w-full text-lg lg:flex 2xl:text-[19px]"
                ref={(e) => {
                  navLinks.current[3] = e;
                }}
              >
              ...
              <li
                key="li-element-5"
                id="search"
                className="inline w-full md:text-neutral-400 text-white 2xl:text-[19px]"
                ref={(e) => {
                  navLinks.current[4] = e;
                }}
              >
                <Link href="/search">
              ...
              <div
                ref={indicatorBar}
                className="hidden md:block absolute -bottom-[5px] w-0 h-0.5 bg-white rounded-lg left-0 duration-200 ease-in-out"
              ></div>
            </ul>
          </nav>
