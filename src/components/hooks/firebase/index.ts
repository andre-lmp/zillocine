...
import {
  CommentProps,
  ReplyProps,
} from "@/components/pages/playerPage/main/commentsSection";
...
const firebaseErrorMessages = {
  ...
  "auth/user-not-found":
    "Nenhuma conta foi encontrada com este endereço de e-mail.",
  "auth/wrong-password": "A senha fornecida está incorreta.",
  "auth/too-many-requests":
    "Muitas solicitações foram feitas. Por favor, tente novamente mais tarde.",
  ...
  "auth/user-token-expired":
    "O token do usuário expirou. Por favor, faça login novamente.",
  ...
  "auth/credential-already-in-use":
    "As credenciais fornecidas já estão em uso.",
  "auth/invalid-phone-number": "O número de telefone fornecido é inválido.",
  "auth/phone-number-already-exists":
    "O número de telefone fornecido já está em uso por outra conta.",
  "auth/quota-exceeded": "O limite de cota foi excedido.",
  "auth/invalid-persistence-type":
    "O tipo de persistência fornecido é inválido.",
  ...
  "auth/reset-password-exceed-limit":
    "O limite de redefinição de senha foi excedido.",
};
...
export default function useFirebase() {
  ...
  const updateUserContext = async (user: UserInterface) => {
    try {
      ...
      console.error(
        "Erro ao atualizar o contexto com os dados do usuario" + error,
      );
    }
  };
  ...
  const getUserImageOnStorage = async (storageRef: any): Promise<void> => {
    ...
    if (user) {
      try {
        ...
        throw new Error(
          "Erro ao buscar ou atualizar a imagem de perfil" + error,
        );
      }
    }
  };
      ...
      try {
        ...
        if (error instanceof FirebaseError) {
          globalEvents.setModalsController((prev) => ({
            ...
            formInstructionsMessage:
              firebaseErrorMessages[
                error.code as keyof typeof firebaseErrorMessages
              ] || "Faça login novamente para encerrar sua conta.",
          }));
        } else {
  ...
  const deleteCurrentUserOnDb = async (userId: string): Promise<boolean> => {
    const db = getDatabase(app);
    const userRef = getDatabaseRef(db, `users/${userId}`);

    try {
      // Remover usuário do Realtime Database
      await remove(userRef);

      // Enviar requisição para o endpoint adicional
      const response = await fetch(
        process.env.NEXT_PUBLIC_DATABASE_URL as string,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          // body: JSON.stringify({ userId }), // Adicionando o `userId` no corpo, se necessário
        },
      );
      ...
      console.error(
        `Erro ao deletar dados do usuário no Firebase Realtime Database: ${error}`,
      );
      return false;
    }
  };
  ...
  const deleteCurrentUserOnStorage = async (userId: string) => {
    ...
    try {
      ...
      console.error(
        "Erro ao deletar dados do usuário no firebase storage" + error,
      );
    }
  };
    ...
    try {
      ...
      if (error instanceof FirebaseError) {
        ...
        const customErrorMessage =
          firebaseErrorMessages[
            error.code as keyof typeof firebaseErrorMessages
          ] || "Não foi possível desconectar sua conta, tente novamente";
  ...
  const registerUser = async (
    ...
    try {
      ...
      if (response && auth.currentUser) {
        ...
        const userName = await extractName(
  ...
  const updateUserData = async (
    ...
    try {
      ...
      if (newEmail && newEmail !== userData.email) {
        const response = await fetch(
          "https://findonemailslist-6lpci3axsq-uc.a.run.app",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (response.ok) {
          ...
          globalEvents.setModalsController((prev) => ({
            ...prev,
            verificationErrorMessage: !isUnique
              ? "Já existe uma conta vinculada a este email"
              : null,
          }));
      ...
      if (auth.currentUser) {
        ...
        if (newName && newName !== auth.currentUser.displayName) {
          ...
          if (auth.currentUser.displayName) {
            const extractedName = await extractName(
      ...
      try {
        ...
        if (error instanceof FirebaseError) {
          ...
          globalEvents.setModalsController((prev) => ({
            ...
            formInstructionsMessage:
              "Faça login novamente para solicitar um link de atualização de email",
          }));
        }
      }
    }
  ...
  const addUserFavoritesToDb = async (
    contentId: string,
    contentType: string,
  ): Promise<void> => {
    const db = getDatabase(app);
    ...
    try {
      ...
      if (userDataOnDb) {
        if (contentType === "movie") {
          const updatedData = userDataOnDb.favoriteMovies
            ? [contentId, ...userDataOnDb.favoriteMovies]
            : [contentId];
        ...
        if (contentType === "serie") {
          const updatedData = userDataOnDb.favoriteSeries
            ? [contentId, ...userDataOnDb.favoriteSeries]
            : [contentId];
      ...
      throw new Error(
        "Erro ao adicionar item aos favoritos do usuario" + error,
      );
    }
  };

  const deleteUserFavoritesOnDb = async (
    contentId: string,
    contentType: string,
  ): Promise<void> => {
    const db = getDatabase(app);
    ...
    try {
      ...
      if (userDataOnDb) {
        if (contentType === "movie" && userDataOnDb.favoriteMovies) {
          const updatedData = userDataOnDb.favoriteMovies.filter(
        ...
        if (contentType === "serie" && userDataOnDb.favoriteSeries) {
          const updatedData = userDataOnDb.favoriteSeries.filter(
            (id: string) => id !== contentId,
          );
  ...
  const addUserCommentsToDb = async (
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        await update(commentIdsListRef, {
          idsList: [...list.idsList, commentData.id],
        });
      } else {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        return new Promise((resolve, reject) => {
          try {
            Promise.all(
              list.idsList.map(async (commentId: string) => {
  ...
  const getUserReactionOnDb = async (
    ...commentsIds: string[]
      ...
      try {
        Promise.all(
          commentsIds.map(async (id) => {
            const reactionRef = getDatabaseRef(
              db,
              `reactions/${id}/${user?.uid}`,
            );
            const userReaction = await get(reactionRef);
            return {
              id: id,
              userReaction: userReaction.exists()
                ? userReaction.val().reaction
                : null,
            };
          }),
        ).then((reaction) => {
  ...
  const updateUserReactionOnDb = async (
    commentId: string,
    reaction: string,
  ) => {
    const user = auth.currentUser;
    ...
    try {
      ...
      if (
        (prevUserReaction && prevUserReaction.reaction !== reaction) ||
        !prevUserReaction
      ) {
      ...
      const response = await fetch(
        "https://updatereactionscount-6lpci3axsq-uc.a.run.app",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      throw new Error(
        "Erro ao adicionar reação do usuario ao banco de dados" + error,
      );
    }
  };
  ...
  const addUserReplyToDb = async (replyData: ReplyProps) => {
    const db = getDatabase(app);
    const commentRef = getDatabaseRef(
      db,
      `replies/${replyData.replyingId}/${replyData.id}`,
    );
    ...
    try {
      ...
      const response = await fetch(
        process.env.NEXT_PUBLIC_DATABASE_URL as string,
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            commentId: replyData.replyingId,
